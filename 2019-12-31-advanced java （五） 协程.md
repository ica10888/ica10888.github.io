...
可以在执行完上一步后，将此步的Continuation 保存下来，假设我们有能力在此步中断，然后在上次中断处重新开始执行 ，就可以去做其他事情。
...
;;; (display 3)  (lambda (x) x) 相当于传入一个 t -> t 匿名函数 ,但是这里 ((lambda (x) x) 2) 只是一个过程，最后的返回值是3
;;; 最后一步的返回值是3，于是就直接打印3


(display (call-with-current-continuation f)) ; displays 2
;;; (display ((lambda (x) x) 2))
;;; callcc 打印的是 (return 2) 的返回值，即执行回调的地方，t -> t 可以看作一个callback函数
;;; callcc 是编译器里面实现
;;; retrun 在scheme里不是关键字或者方法，这里只是传入参数的命名
```

在调用 `f` 时，会调用并使这个`f`的参数 `lambda 函数` 获取值2，但是最后会返回3。

当使用callcc 的时候，入参 (continuation)，程序将回到执行回调的地方，即返回值2，这也就是打印的值。
...
call/cc 具有改变控制流的能力，即可以跳到任意执行回调的地方。这样，多个回调就可以拆开，我们将回调作为中断处，有了从中断处重新开始执行的能力。
...
(say 1 (lambda(x) 
   (display (string-append "第" (number->string x) "次执行\n"))
   (+ x 1)
))
```
...
（lambda 演算是等价于图灵机的，这种计算机程序和数学证明之间的紧密联系 ，被称作Curry-Howard同构）
