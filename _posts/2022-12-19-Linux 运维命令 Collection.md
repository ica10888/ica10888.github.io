...
- 内存
  ...
  **eBPF（extended Berkeley Packet Filters）机制**：是一种内核技术，它允许开发人员在不修改内核代码的情况下运行特定的功能。 
...
- 网络io
  ...
  **TCP 重传**：超时重传，快速重传，SACK，D-SACK
  
  **TCP滑动窗口**: 发送窗口，接收窗口
  
  **TCP流量控制**: 操作系统缓冲区与滑动窗口的关系，窗口关闭，糊涂窗口
  
  **TCP拥塞控制**: 慢启动，阻塞避免，阻塞发生，快速恢复
...
| ifstat      | 统计网络接口活动状态                                         | -tT 更详细的报告<br/>-a 监控所有网络接口<br/>                |
...
| ethtool     | 查询和控制网络设备驱动程序和硬件设置                         | -i 显示网卡驱动的信息，如驱动的名称、版本等。<br/>-k 显示网卡Offload参数的状态：on 或 off，包括rx-checksumming、tx-checksumming等。<br/>-a 查看网卡中 接收模块RX、发送模块TX和Autonegotiate模块的状态：启动on 或 停用off。<br/>-p 用于区别不同ethX对应网卡的物理位置，常用的方法是使网卡port上的led不断的闪；N指示了网卡闪的持续时间，以秒为单位。<br/>-s 修改网卡的部分配置，包括网卡速度、单工/全双工模式、mac地址等。 |
...
| dmesg    | 显示linux信息<br/>注：包括一些内存异常信息的日志，有时候排查问题找不到思路的时候，这个指令有奇效 |                                                              |
| nslookup | 域名分析                                                     |                                                              |
